...
def load_GOES_ts_seven():
    train_data = GOES_ts_seven(train = True, seq_len = cfg.in_len + cfg.out_len)
    test_data = GOES_ts_seven(train = False, seq_len = cfg.in_len + cfg.eval_len)
    valid_data = test_data
    return train_data, valid_data, test_data


# load RGB composite of OCTANE output AMVs derived from ABI (GOES-16, M2)
def load_file(data_root, data_type, seq_len, T):    # T is n_skip, or number of frames to skip (should probably be just 1 for now)
    if data_type == 'test':
    ...
    print('Total sub-sequences: %d' % len(seq_list))
    print('Total sequences: %d' % len(clip_dict))
