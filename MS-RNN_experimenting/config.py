...
# I intend to modify at least a couple of the files from this repo, however it is apparent that it has given
# me an invaluable starting point for working with these techniques (i.e. MS-LSTM).
...
cfg = edict()
cfg.model_name = 'MS-ConvLSTM'
cfg.gpu = '0'   # only have one GPU at a time
cfg.gpu_nums = len(cfg.gpu.split(','))
cfg.work_path = 'MS-RNN_experimenting'  # BD
cfg.dataset = "GOES-ABI_test-set-7"  # moving-mnist-20  kth_160_png  taxiBJ  HKO-7-180-with-mask  MeteoNet-120  DWD-12-480  RAIN-F
...
# BD custom config for working with custom data/dataset
    ...
    if 'test-set-5' in cfg.dataset:
        cfg.width = 256     # 256 instead of an even division of 500 (original image size) due to the RNN Upsampling process (doesn't work if division ends up in non-integers)
        cfg.height = 256 * 3    # * 3 due to preprocessing steps (stacking R G and B vertically)
        cfg.in_len = 6
        cfg.out_len = 6
        cfg.eval_len = 6
        cfg.epoch = 10
